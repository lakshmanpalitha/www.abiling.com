...
class advadloadclass {
    ...
    public function __construct($id=false,$ref='N') {
        ...
        if($ref=='Y'){
            $this->userId = $id;
        }else if ($this->pro->getSession("advac")) {
        ...
        $this->advsum = new advsummary($this->userId);
    ...
    public function loadAds() {
        $available_ads = $this->con->queryMultipleObjects("SELECT * FROM adviewer_view_ads a_v_a,ads_fool af WHERE a_v_a.isblock=0 AND af.del_ad=0 AND af.isblock=0 AND a_v_a.account_id='" . $this->userId . "' AND a_v_a.ads_id=af.ads_id ORDER BY a_v_a.ads_id LIMIT " . $this->no_of_ads_per_round . "");
        if (!$available_ads) {
            return false;
        }
...
//        $available_oads = $this->con->queryMultipleObjects("SELECT * FROM adviewer_view_ads a_v_a,ads_fool af WHERE NOT EXISTS(SELECT ads_id FROM adviewer_view_ads WHERE adviewer_view_ads.ads_id=a_v_a.ads_id AND (adviewer_view_ads.temp_block=1 AND  isblock=0)OR (adviewer_view_ads.temp_block=0 AND  isblock=0)) AND a_v_a.isblock=1 AND af.del_ad=0 AND af.isblock=0 AND a_v_a.account_id='" . $this->userId . "' AND  a_v_a.ads_id=af.ads_id GROUP BY  a_v_a.ads_id  ORDER BY  RAND() LIMIT 25 ");
//        if (!$available_oads) {
//
//            return false;
//        }
...
//       
        ...
        $this->con->execute("UPDATE adviewer_view_ads a_v_a 
                                            ...
                                            WHERE a_v_a.account_id='" . $this->userId . "'");

        $this->con->execute("UPDATE adviewer_register SET isstartrounddate=0 WHERE account_id='" . $this->userId . "' ");
...
//  
    ...
    public function setAd() {
        ...
        $isstartrounddate = $this->con->queryUniqueValue("SELECT isstartrounddate FROM adviewer_register WHERE account_id='" . $this->userId . "'");
        if ($isstartrounddate == 0) {
...
//                    WHERE adviewer_view_ads.ads_id = ads_fool.ads_id AND ads_fool.isblock=0 AND ads_fool.del_ad=0 AND adviewer_view_ads.account_id='" . $this->userId . "')");
            ...
            $adsFool = $this->con->queryMultipleObjects("SELECT ads_id AS id FROM ads_fool WHERE del_ad=0 AND isblock=0");
            ...
            $rand_keys = false;
//              var_dump($commonAds);
            if ($no_of_val > $this->no_of_ads_per_round) {
                $nrand_keys = array_rand($commonAds, $this->no_of_ads_per_round);
//                var_dump($nrand_keys);
                $myads = array();
                foreach ($nrand_keys as $c) {
                    array_push($myads, $commonAds[$c]);
                }
                $rand_keys = $myads;
            } else {
                $rand_keys = $commonAds;
...
//        }
            ...
            $this->con->execute("UPDATE adviewer_register SET round_date='" . $this->date . "',isstartrounddate=1 WHERE account_id='" . $this->userId . "' ");
        }
    }
